...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "UP_UO":
      try:
         ...
         await client.set_administrator_title(chat_id, user_id, "مودي")
      except:
        pass
      return await message.reply_text(f"**♪ انضم المطور مودي للشات  💎 .\n♪ مرحبا بك : @UP_UO  💎 .**")
    dev = await get_dev(bot_username)
...
async def UP_UO(client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
...
"مره واحد اسمو مودي  كان بيرجم ابليس ف الحج قالولو ليه؟ قال عشان يمكن احتاجو 👻😹",
...
"لا مودي على الدوق فيما يحب أو بكره",
...
async def alive(client: Client, message):
    ...
    alive = f"""╭──── • ◈ • ────╮
么 [𝐬𝐨𝐮𝐫𝐜𝐞 𝒛𝒆](t.me/UI_XB) 💎 .
么  [𝒅𝒆𝒗 𝒎𝒐𝒅𝒚](t.me/UP_UO) 💎 .
╰──── • ◈ • ────╯
...
@Client.on_message(filters.command(["المطور مودي","مودي","المبرمج"], ""))
async def deev(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="UP_UO")
...
@Client.on_message(filters.command(["المودي","المطور جيكا ","المطور مودي"], ""))
...
async def set_vi_so(client: Client, message):
   NAME = await client.ask(message.chat.id,"**♪ ارسل رابط لوجو السورس  💎 .\n♪ مثال ⟨ https://telegra.ph/file/7065a4c7cbedef8befdb9.jpg ⟩  💎 .**", filters=filters.text, timeout=30)
   VID_SO = NAME.text
   bot_username = client.me.username
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["UP_UO"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["UP_UO"]:
   ...
   if message.from_user.username == "UP_UO":
     await message.reply_text(f"**♪ تم تنفيذ الامر بواسطة المطور مودي  💎 .**")
   else:
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @UI_XB  💎 .**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @UI_XB  💎 .**")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @UI_XB  💎 .**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in OWNER :
         ...
         return
     if userr.username in ["UP_UO"]:
         await message.reply_text("**♪ رتبتك هي : المطور مودي  💎 .**")
         return
     if userr.username in ["UP_UO"]:
         await message.reply_text("**♪ رتبتك هي : المطور مودي 💎 .**")
         return
